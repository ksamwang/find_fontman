...
if ([string]::IsNullOrWhiteSpace($Fonts)) {
    $Fonts = (Get-ChildItem -Path "fonts" -Directory | Where-Object { $_.Name -like "1*" } | Select-Object -First 1).FullName
}

if ([string]::IsNullOrWhiteSpace($Fonts)) {
    throw "Could not find the Chinese simplified fonts directory under fonts\."
}
